...
def enable_change_levels():
    from gamesrv import BB_DIR
    f = open(os.path.join(BB_DIR, 'levels', 'ALL_LEVELS'), 'r')
    all_level_files = [p.strip() for p in f.readlines()]
    f.close()
    gamesrv.game.FnChangeLevels = [p for p in all_level_files
                                     if p and not p.startswith('#')]

def disable_change_levels():
    gamesrv.game.FnChangeLevels = None
...
def next_board(num=0, complete=1, fastreenter=False):
    ...
    if not inplace:
        ...
        # wait for at least one player before entering the first board
        if num <= 0:
...
def normal_frame():
    ...
    if not (BubPlayer.FrameCounter & 31):
        ...
        if reset and BubPlayer.FrameCounter >= reset:
            ...
            set_frametime(1.0)
    if any_player != (gamesrv.game.FnChangeLevels is None):
        global _changelevels_delay
        _changelevels_delay += 1
        if _changelevels_delay >= 5:
            _changelevels_delay = 0
            if any_player:
                disable_change_levels()
            else:
                enable_change_levels()
...
def extra_make_random_level(cx=None, cy=None, repeat_delay=200):
    ...
    filename = os.path.join(BB_DIR, 'levels', 'RandomLevels.py')
    d = {'__name__': 'bubbob._randomlevels_'}
    execfile(filename, d)
